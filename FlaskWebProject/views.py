...
def upload():
    ...
    return render_template("displayPhotos.html",items=items, userID = userID)
    """
    #only for images
...
def delete():
    ...
    for item in db.fs.files.find({"username":userID, "filename" : filename}):
        #print item['_id']
        removechunk = db.fs.chunk.remove({"files_id": item['_id']})
